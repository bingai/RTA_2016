...
def main():
    ...
    raw_input("== When ready to calibrate tactile sensors, press [Enter]\n")
    calibrate_tactile()
    raw_input("Calibrations are done!")

    # Demonstration of position control
    raw_input("== When ready to wiggle fingers with position control, hit [Enter]\n")
    pos_pub.publish(PoseCommand(f1=1.57, f2=1.57, f3=1.57, preshape=0))
    rospy.sleep(0.1)

    # while True:
    #     print("== When ready to wiggle fingers with position control, hit [Enter]\n")
    #     angle = float(raw_input("Enter position:"))
    #     pos_pub.publish(PoseCommand(f1=1, f2=1, f3=1, preshape=angle))
    #     rospy.sleep(0.1)

    enable_tactile_stops()
    raw_input("Closing ... [Enter]\n")
    # f1 = FingerPressure([10, 10, 10, 10, 10, 40, 40, 40, 1000])
    # f2 = FingerPressure([10, 10, 10, 10, 10, 40, 40, 40, 1000])
    # f3 = FingerPressure([20, 20, 20, 20, 20, 40, 40, 40, 1000])
    f1 = FingerPressure([50, 50, 50, 50, 50, 50, 50, 50, 10])
    f2 = FingerPressure([50, 50, 50, 50, 50, 50, 50, 50, 10])
    f3 = FingerPressure([50, 50, 50, 50, 50, 50, 10, 10, 10])
    threshold = SetTactileThresholdRequest([f1, f2, f3])
    set_tactile_threshold(threshold)
    vel_pub.publish(VelocityCommand(f1=0.5, f2=0.5, f3=0.5, preshape=0.0))
    rospy.sleep(3.0)
    raw_input("... [Enter]\n")
    pos_pub.publish(PoseCommand())
