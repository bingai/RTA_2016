...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle n;
  joint_pub = n.advertise<sensor_msgs::JointState>("/robot/joint_states", 1);
  ...
  for (int finger = 1; finger<4; finger++)
  {
    ...
    {
      if (i == 1)
        sprintf(buffer, "finger_%d_/flex_joint_from_prox_to_1", finger);
      else if (i == (NUM_FLEX_STEPS+1))
        sprintf(buffer, "finger_%d_/flex_joint_from_%d_to_dist", finger, NUM_FLEX_STEPS);
      else
        sprintf(buffer, "finger_%d_/flex_joint_from_%d_to_%d", finger, i-1, i);
