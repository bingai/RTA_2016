...
bool fitMinimumVolumeBoundingBoxConstrained(const typename pcl::PointCloud<PointT>::ConstPtr &cloud, const Eigen::Vector4f &support_plane, Eigen::Affine3f &pose, Eigen::Vector3f &size) {
	...
	// Get table plane parameters
	Eigen::Vector3f planePoint, planeNormal;
	utl::geom::planeCoefficientsToPointNormal(support_plane, planePoint, planeNormal);

	// Get the convex hull of the input points projected on the table plane
	typename pcl::PointCloud<PointT>::Ptr hull(new pcl::PointCloud<PointT>);
	utl::cloud::ConvexHull2D<PointT> ch2d;
	ch2d.setInputCloud(cloud);
	ch2d.setPlaneCoefficients (support_plane);
	ch2d.reconstruct(*hull);

	// Find the minimum bounding rectangle for the convex hull
	Eigen::MatrixXf hull_pts = hull->getMatrixXfMap().topRows(3);
	Eigen::MatrixXf obj_pts = cloud->getMatrixXfMap().topRows(3);

	Eigen::Vector3f z_dir = planeNormal;
